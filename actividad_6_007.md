...
- `git stash` guarda un **snapshot de tus cambios sin commitear** (staged y unstaged) en un área especial llamada _stash_, y devuelve tu directorio de trabajo a un estado limpio.
...
Esta actividad incluye una evaluación automática similar a la de la actividad anterior, adaptada a `git stash` y gestión de trabajo temporal:
...
- `required_files` (por defecto: `tarea.txt,cp1-estado-inicial.txt,cp2-stash-list.txt,cp3-apply-vs-pop.txt,cp4-multiples-stash.txt,reflexion-6-7.md`)
