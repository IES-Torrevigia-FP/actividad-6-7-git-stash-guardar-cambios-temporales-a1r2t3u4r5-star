...
def analyze_stash_activity(root: Path) -> dict:
    cp1_path = root / 'cp1-estado-inicial.txt'
    cp2_path = root / 'cp2-stash-list.txt'
    cp3_path = root / 'cp3-apply-vs-pop.txt'
    cp4_path = root / 'cp4-multiples-stash.txt'
    cp5_path = root / 'cp5-untracked.txt'
    tarea_path = root / 'tarea.txt'
    ...
    tarea = safe_read_text(tarea_path).lower()

    cp1_estado_ok = contains_any(cp1, ['git status', 'modified', 'cambios'])
    cp1_reason_ok = contains_any(cp1, ['incomplet', 'todavia', 'todavía', 'aun', 'aún', 'no quiero'])

    cp2_stash_list_ok = contains_any(cp2, ['git stash list', 'stash@{'])
    cp2_log_all_ok = contains_any(cp2, ['git log --oneline --all', 'aplica hotfix urgente', 'hotfix'])
    cp2_explain_ok = contains_any(cp2, ['guardad', 'stash', 'temporal'])

    cp3_apply_ok = contains_any(cp3, ['git stash apply', 'apply'])
    cp3_pop_ok = contains_any(cp3, ['git stash pop', 'pop'])
    cp3_diff_ok = contains_any(cp3, ['diferencia', 'mantiene', 'elimina']) and contains_any(cp3, ['apply', 'pop'])

    cp4_multi_ok = contains_any(cp4, ['stash@{0}', 'stash@{1}', 'git stash list'])
    cp4_specific_ok = contains_any(cp4, ['git stash apply stash@{', 'stash específico', 'stash especifico'])
    cp4_messages_ok = contains_any(cp4, ['mensaje', 'descriptivo'])

    cp5_optional_ok = False
    if cp5_path.exists():
        cp5_optional_ok = contains_any(cp5, ['git stash -u', '--include-untracked']) and contains_any(cp5, ['git stash pop', 'diferencia'])

    tarea_min_content_ok = contains_any(tarea, ['tarea principal']) or contains_any(tarea, ['login'])

    return {
        'cp1_estado_inicial': {
            'ok': cp1_estado_ok and cp1_reason_ok,
            'source': 'cp1-estado-inicial.txt' if cp1_path.exists() else 'missing',
        },
        'cp2_stash_list': {
            'ok': cp2_stash_list_ok and cp2_log_all_ok and cp2_explain_ok,
            'source': 'cp2-stash-list.txt' if cp2_path.exists() else 'missing',
        },
        'cp3_apply_pop': {
            'ok': cp3_apply_ok and cp3_pop_ok and cp3_diff_ok,
            'source': 'cp3-apply-vs-pop.txt' if cp3_path.exists() else 'missing',
        },
        'cp4_multiples_stash': {
            'ok': cp4_multi_ok and cp4_specific_ok and cp4_messages_ok,
            'source': 'cp4-multiples-stash.txt' if cp4_path.exists() else 'missing',
        },
        'cp5_untracked_opcional': {
            'present': cp5_path.exists(),
            'ok': cp5_optional_ok,
            'source': 'cp5-untracked.txt' if cp5_path.exists() else 'missing',
        },
        'tarea_final': {
            'exists': tarea_path.exists(),
            'minimum_content_ok': tarea_min_content_ok,
        },
    }
...
def main():
    ...
    ap.add_argument('--required', default='tarea.txt,cp1-estado-inicial.txt,cp2-stash-list.txt,cp3-apply-vs-pop.txt,cp4-multiples-stash.txt,reflexion-6-7.md')
    ...
    stash_info = analyze_stash_activity(root)
    ...
    reflex_path = root / 'reflexion-6-7.md'
    ...
    evidencia_candidates = [
        'cp1', 'cp2', 'cp3', 'cp4', 'cp5',
        'stash', 'apply', 'pop', 'drop', 'untracked', 'reflexion',
    ]
    ...
    checks_stash = [
        stash_info['cp1_estado_inicial']['ok'],
        stash_info['cp2_stash_list']['ok'],
        stash_info['cp3_apply_pop']['ok'],
        stash_info['cp4_multiples_stash']['ok'],
        stash_info['tarea_final']['exists'] and stash_info['tarea_final']['minimum_content_ok'],
    ]
    tech_hits = sum(1 for x in checks_stash if x)
    ...
    total = s_git_stash + s_evid + s_reflex
    ...
    with (outdir / 'metricas.csv').open('w', newline='', encoding='utf-8') as f:
        ...
        w.writerow(['git_stash (max 4)', s_git_stash])
    ...
    md.append('|---|---:|')
    md.append(f"| Uso de git stash (stash, list, apply, pop, drop) | {s_git_stash}/4 |")
    md.append(f"| Evidencias (checkpoints + archivos) | {s_evid}/3 |")
    md.append(f"| Reflexion 6.7 | {s_reflex}/3 |")
    ...
    md.append('## Checks de stash')
    md.append(f"- Estado inicial con cambios y justificacion (cp1): {'si' if stash_info['cp1_estado_inicial']['ok'] else 'no'}  ")
    md.append(f"- Stash list + log all + explicacion (cp2): {'si' if stash_info['cp2_stash_list']['ok'] else 'no'}  ")
    md.append(f"- Apply vs pop correctamente explicado (cp3): {'si' if stash_info['cp3_apply_pop']['ok'] else 'no'}  ")
    md.append(f"- Multiples stashes y aplicacion especifica (cp4): {'si' if stash_info['cp4_multiples_stash']['ok'] else 'no'}  ")
    md.append(f"- Checkpoint opcional untracked (cp5): {'si' if stash_info['cp5_untracked_opcional']['ok'] else 'no'}  ")
    md.append(f"- tarea.txt final presente y minimo valido: {'si' if stash_info['tarea_final']['exists'] and stash_info['tarea_final']['minimum_content_ok'] else 'no'}  ")
    md.append('')
