...
Extracto Reflexion 6.7 (max 8k chars, si existe):
```
{reflex_excerpt}
```
...
- Esta actividad (6.7) se califica con rubrica: Git stash (max 4), Evidencias (max 3), Reflexion (max 3).
- No incluyas README como criterio de calificacion ni como area de mejora principal.
- Si en JSON aparece stash_checks.cp2_stash_list.ok en true, reconoce explicitamente la evidencia de `git stash list`.
- Si en JSON aparece stash_checks.cp3_apply_pop.ok en true, reconoce explicitamente la diferencia entre `apply` y `pop`.
- Si en JSON aparece stash_checks.cp4_multiples_stash.ok en true, reconoce explicitamente el manejo de stashes multiples.
- Si en JSON aparece stash_checks.cp5_untracked_opcional.ok en true, reconoce explicitamente el uso de `git stash -u`.
- Espanol formal.
...
def main():
    ...
    reflex_excerpt = read_opt(Path('reflexion-6-7.md'))
